...
Built-in script at `${CODEX_HOME:-$HOME/.codex}/skills/read/scripts/fetch_feishu.py`. Requires `requests` and Feishu app credentials:

```bash
pip install requests  # one-time setup
export FEISHU_APP_ID=your_app_id
export FEISHU_APP_SECRET=your_app_secret
python3 "${CODEX_HOME:-$HOME/.codex}/skills/read/scripts/fetch_feishu.py" "{url}"
```
...
python3 "${CODEX_HOME:-$HOME/.codex}/skills/read/scripts/fetch_weixin.py" "{url}"
```
